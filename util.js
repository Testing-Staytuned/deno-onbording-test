...
async function appendToIssueDescription(additionalContent) {
  ...
  const existingDescription = await fetchCurrentDescription(
    owner,
    repo,
    issueNumber,
    token
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to append content to issue description: ${response.statusText}`
      );
    }
...
async function fetchCurrentDescription(owner, repo, issueNumber, token) {
  ...
  try {
    ...
    if (!descriptionResponse.ok) {
      throw new Error(
        `Failed to fetch issue description: ${descriptionResponse.statusText}`
      );
    }
